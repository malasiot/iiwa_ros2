...
def generate_aruco(aruco_ids, dict_type):
    # Get params
    ...
    output_folder = os.path.join(os.getcwd(), "aruco", dict_type)
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    # load the dae template
    with open(os.path.join(os.getcwd(), 'aruco', 'template.dae'), mode='r') as file:
        dae_template = file.read()

    for aruco_id in aruco_ids:
        print("generating ArUCo tag type '{}' with ID '{}'".format(
            dict_type, aruco_id))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='generate_aruco_meshes')
    parser.add_argument('ids')           # positional argument
    parser.add_argument('dict')      # option tha takes a value
	
    args = parser.parse_args()

    generate_aruco(args.ids, args.dict)
